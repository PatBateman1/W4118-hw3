...
static void dieThread(const char *message)
{
    perror(message);
    pthread_exit(NULL);
}
...
void * handleRequest(void *arg)
{
    ...
    FILE *clntFp = fdopen(param->clntSock, "r");
    if (clntFp == NULL)
        dieThread("fdopen failed");
    ...
    */
    ...
    if (!method || !requestURI || !httpVersion || 
        ...
        sendStatusLine(param->clntSock, statusCode);
        goto handle_end;
    }
    ...
    */
    ...
    statusCode = handleFileRequest(param->webRoot, requestURI, param->clntSock);
...
handle_end:
    ...
    */
    ...
    fprintf(stderr, "%s \"%s %s %s\" %d %s\n",
            inet_ntop(AF_INET, &(param->clntAddr->sin_addr), buffer, 20),
            method,
            requestURI,
            httpVersion,
            statusCode,
...
int main(int argc, char *argv[])
{
    ...
    for (;;) {
        ...
        param->clntSock = clntSock;
        param->clntAddr = &clntAddr;
