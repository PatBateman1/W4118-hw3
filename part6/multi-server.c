...
void * handleRequest(void *arg)
{
    ...
    for (;;) {
        ...
        int clntSock = accept(param->servSock, (struct sockaddr *)&clntAddr, &clntLen);
        if (clntSock < 0)
            die("accept() failed");
        ...
        statusCode = handleFileRequest(param->webRoot, requestURI, clntSock);
