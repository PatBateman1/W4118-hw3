        ...
        const char *webRoot, const char *requestURI, int clntSock, sem_t *semptr, struct data *data)
{
    ...
    if (strcmp(requestURI, "/statistics") == 0) {
        // kill(getpid(), SIGUSR1); for testing
        statusCode = 200;
        while (sem_wait(semptr) == -1) {
            if (errno == EINTR) 
                continue;
    ...
    if (stat(file, &st) == 0 && S_ISDIR(st.st_mode)) {
        ...
        if (pipe(fd) < 0)
            die("pipe() failed");
        if ((pid = fork()) < 0) {
            die("fork() failed from directory listing");
        } else if (pid == 0) {
            // child
            close(fd[0]);

            // redirect standard output to the pipe write end
            if (dup2(fd[1], STDOUT_FILENO) != STDOUT_FILENO) 
                die("dup2() failed");

            // redirect standard error to the pipe write end
...
void handler(int sig)
{
    ...
    snprintf(stats, 1000,
            ...
            "4xx: %d\n"
            "5xx: %d\n", data->stat[0], data->stat[1], data->stat[2], data->stat[3], data->stat[4]);
    if (sem_post(&data->semptr) == -1)
        die("sem_post() failed");
    if (write(STDERR_FILENO, stats, strlen(stats)) != strlen(stats))
        die("write() failed");
}
...
int main(int argc, char *argv[])
{
    ...
    if (pid == 0) {
        ...
        for (;;) {
            ...
            */
            int clntSock = recvConnection(fd[0]);
    ...
    loop_end:
        ...
        for (;;) {
            ...
            */
            ...
            if (clntSock < 0)
                die("accept() failed");
            sendConnection(clntSock, socks[index % WORKER_PROCESS]);
            index++;
            close(clntSock);
        }
    }
